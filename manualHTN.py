...
def op_craft_plank_at_bench(state, ID):
	if state.time[ID] >= 1 and state.bench[ID] >= 1 and state.wood[ID] >= 1:
		state.plank[ID] += 4
		state.wood[ID] -= 1
		state.time[ID] -= 1
		return state
...
def op_craft_stick_at_bench(state, ID):
	if state.time[ID] >= 1 and state.bench[ID] >= 1 and state.plank[ID] >= 1:
		state.stick[ID] += 4
		state.plank[ID] -= 1
		state.time[ID] -= 1
		return state
...
pyhop.declare_operators(op_punch_for_wood, op_craft_wooden_axe_at_bench, op_wooden_axe_for_wood, op_craft_plank_at_bench, op_craft_stick_at_bench, op_craft_plank, op_craft_stick, op_craft_bench)
...
def craft_plank_at_bench(state, ID):
	return [('have_enough', ID, 'bench', 1), ('have_enough', ID, 'wood', 1), ('op_craft_plank_at_bench', ID)]

def craft_stick_at_bench(state, ID):
	return [('have_enough', ID, 'bench', 1), ('have_enough', ID, 'plank', 1), ('op_craft_stick_at_bench', ID)]
...
pyhop.declare_methods('produce_plank', craft_plank, craft_plank_at_bench)
pyhop.declare_methods('produce_stick', craft_stick, craft_stick_at_bench)
